...
class AnchorBoxes(Layer):
    ...
    def call(self, x, mask=None):
        ...
        Arguments:
            ...
            x (tensor):
                4D tensor of shape `(batch, height, width, channels)`.
                The input for this layer must be the output of the localization
                predictor layer.
        '''
        ...
        for ar in self.aspect_ratios:
            if ar == 1:
                ...
                if self.two_boxes_for_ar1:
                    ...
                    box_dim = np.sqrt(self.this_scale * self.next_scale) * size
                    wh_list.append([box_dim]*2)
            else:
                wh_list.append([
                    self.this_scale * size * np.sqrt(ar),
                    self.this_scale * size / np.sqrt(ar)
                ])
        wh_list = np.array(wh_list)

        # We need the shape of the input tensor
        batch_size, feature_map_height, feature_map_width, _ = x.shape
        ...
        if self.this_steps is None:
        ...
        elif (isinstance(self.this_steps, (list, tuple))
                and len(self.this_steps) == 2):
            ...
            raise TypeError(
                '`this_steps` has a non-standard format: {}'.format(
                    self.this_steps)
            )
        ...
        if self.this_offsets is None:
            offset_height = 0.5
            offset_width = 0.5
        elif (isinstance(self.this_offsets, (list, tuple))
                and (len(self.this_offsets) == 2)):
            ...
            raise TypeError(
                '`this_offsets` has a non-standard format: {}'.format(
                    self.this_offsets)
            )
        ...
        if self.coords in ['centroids', 'minmax']:
            # Convert `(xmin, ymin, xmax, ymax)` back to `(cx, cy, w, h)` or to
            # `(xmin, xmax, ymin, ymax)`.
            boxes_tensor = convert_coordinates(
                boxes_tensor,
                start_index=0,
                conversion='corners2' + self.coords,
                border_pixels='half'
            )
        ...
        boxes_tensor = tf.tile(
            tf.constant(boxes_tensor),
            tf.constant([batch_size, 1, 1, 1, 1])
        )

        return boxes_tensor

    def compute_output_shape(self, input_shape):
        batch_size, fmap_height, fmap_width, _ = input_shape
